#multiple colinear test
#summary convert into data frame and check significance
#dataset:ccpp

library(caret)
library(MASS)
library(corrplot)
path="C:/Users/mayur/Desktop/datascience DELL/data analyst vedios/r language/machine learning/ccpp.csv"

ccpp=read.csv(path,header=T)
...
# EDA
colnames(ccpp)[apply(ccpp,2,checkNull)]
colnames(ccpp)[apply(ccpp,2,checkZero)]

cols=colnames(ccpp)[1:4]
cols

# check the distribution of X
for (c in cols) {
  title= paste("histogram for",c)
  hist(unlist(ccpp[c]),main=title,col='yellow')
}
...
for (c in cols) {
  ...
  boxplot(unlist(ccpp[c]),main= "title",col = "red",horizontal = T)
...
#####
summ_m12=summary(m12)

##### convert the summary into a dataset
# ..................... convert the summary into a dataset .............
# names(summ_m12)
df1 = data.frame(summ_m12$coefficients)
names(df1)=c('beta','stderr','tvalue','pvalue')
df1$x = rownames(df1)
rownames(df1)=NULL
cols=c('x','beta','stderr','tvalue','pvalue')
df1=df1[cols]
df1
# ############# /convert summary to dataset .............................
# significant features based on confidence level
  sigfeatures = function(area,df)
  {
    if (area >= 1)
      area = 0.9999
    
    # get the Z-score for the given area
    z = abs(qnorm(area))
    
    # create a new column in the dataframe to store the significance value
    df$signif = ''
    
    # check the tvalue for each feature for significance by comparing
    # tvalue with z (for the given area)
    for (i in seq(1,nrow(df1)))
    {
      tval = abs(df[i,"tvalue"])
      
      if (tval > z)
        df[i,"signif"]="significant" else
          df[i,"signif"]="insignificant"
    }
    
    return(df[,c('x','signif')])
  }

# test the significance of features
sigfeatures(0.9,df1)
